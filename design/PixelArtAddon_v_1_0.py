...
def single_material(context):
    ...
    material.node_tree.links.new(emissionNode.outputs["Emission"], materialOutput.inputs["Surface"])
    ...
    material.node_tree.links.new(colorRampNode.outputs["Color"], emissionNode.inputs["Color"])
    ...
    material.node_tree.links.new(mixSoftLightNode.outputs["Color"], colorRampNode.inputs["Fac"])
    ...
    material.node_tree.links.new(shaderToRgbNode.outputs["Color"], mixSoftLightNode.inputs["Color1"])
    ...
    material.node_tree.links.new(bsdfNode.outputs["BSDF"], shaderToRgbNode.inputs["Shader"])
    ...
    material.node_tree.links.new(bayerTexNode.outputs["Color"], mixSoftLightNode.inputs["Color2"])
    ...
    material.node_tree.links.new(bayerMapping.outputs["Vector"], bayerTexNode.inputs["Vector"])
    ...
    material.node_tree.links.new(combineXyzNode.outputs["Vector"], bayerMapping.inputs["Vector"])
    ...
    material.node_tree.links.new(multiplyNode1.outputs["Value"], combineXyzNode.inputs["X"])
    ...
    material.node_tree.links.new(multiplyNode2.outputs["Value"], combineXyzNode.inputs["Y"])
    ...
    material.node_tree.links.new(divideNode1.outputs["Value"], multiplyNode1.inputs[0])
    ...
    material.node_tree.links.new(divideNode2.outputs["Value"], multiplyNode2.inputs[1])
    ...
    material.node_tree.links.new(resolutionXnode.outputs["Value"], divideNode1.inputs[0])
    ...
    material.node_tree.links.new(resolutionYnode.outputs["Value"], divideNode2.inputs[0])
    ...
    material.node_tree.links.new(separateXYZnode.outputs["X"], multiplyNode1.inputs[1])
    material.node_tree.links.new(separateXYZnode.outputs["Y"], multiplyNode2.inputs[0])
    ...
    material.node_tree.links.new(texCoordNode.outputs["Window"], separateXYZnode.inputs["Vector"])
...
def multiple_material(context):
    ...
    ditherGroup.links.new(bayerTexNode.outputs["Color"], outputNode.inputs["Color"])
    bayerTexNode.image = bayerMatrix
    bayerTexNode.interpolation = 'Closest'
    ...
    ditherGroup.links.new(bayerMapping.outputs["Vector"], bayerTexNode.inputs["Vector"])

    # Creates the combineXYZ node
    combineXyzNode = ditherGroup.nodes.new(type = "ShaderNodeCombineXYZ")
    combineXyzNode.location = (-800, 0)
    ditherGroup.links.new(combineXyzNode.outputs["Vector"], bayerMapping.inputs["Vector"])

    # Creates two multiply nodes
    multiplyNode1 = ditherGroup.nodes.new(type = "ShaderNodeMath")
    multiplyNode1.location = (-1100, 100)
    multiplyNode1.operation = 'MULTIPLY'
    ditherGroup.links.new(multiplyNode1.outputs["Value"], combineXyzNode.inputs["X"])

    multiplyNode2 = ditherGroup.nodes.new(type = "ShaderNodeMath")
    multiplyNode2.location = (-1100, -100)
    multiplyNode2.operation = 'MULTIPLY'
    ditherGroup.links.new(multiplyNode2.outputs["Value"], combineXyzNode.inputs["Y"])
    ...
    divideNode1.operation = 'DIVIDE'
    divideNode1.inputs[1].default_value = 2
    ditherGroup.links.new(divideNode1.outputs["Value"], multiplyNode1.inputs[0])

    divideNode2 = ditherGroup.nodes.new(type = "ShaderNodeMath")
    divideNode2.operation = 'DIVIDE'
    divideNode2.location = (-1400, -300)
    divideNode2.inputs[1].default_value = 2
    ditherGroup.links.new(divideNode2.outputs["Value"], multiplyNode2.inputs[1])

    # Creates the Resolution nodes
    resolutionXnode = ditherGroup.nodes.new(type = "ShaderNodeValue")
    resolutionXnode.location = (-1700, 200)
    resolutionXnode.label = "ResolutionX"
    ditherGroup.links.new(resolutionXnode.outputs["Value"], divideNode1.inputs[0])
    ...
    resolutionYnode.label = "ResolutionY"
    ditherGroup.links.new(resolutionYnode.outputs["Value"], divideNode2.inputs[0])
    ...
    ditherGroup.links.new(separateXYZnode.outputs["X"], multiplyNode1.inputs[1])
    ditherGroup.links.new(separateXYZnode.outputs["Y"], multiplyNode2.inputs[0])

    # Creates the Texture Coordinate node
    texCoordNode = ditherGroup.nodes.new(type = "ShaderNodeTexCoord")
    texCoordNode.location = (-2000, 50)
    ditherGroup.links.new(texCoordNode.outputs["Window"], separateXYZnode.inputs["Vector"])
    ...
    material.node_tree.links.new(emissionOutputNode.outputs["Emission"], materialOutput.inputs["Surface"])
    ...
    material.node_tree.links.new(mixShaderNode1.outputs["Color"], emissionOutputNode.inputs["Color"])
    ...
    material.node_tree.links.new(mixShaderNode2.outputs["Color"], mixShaderNode1.inputs[2])
    ...
    material.node_tree.links.new(colorRampNode.outputs["Color"], mixShaderNode1.inputs["Color1"])
    colorRampNode.color_ramp.interpolation = 'CONSTANT'
    colorRampNode.color_ramp.elements.remove(colorRampNode.color_ramp.elements[1])
    ...
    material.node_tree.links.new(ditherGroupNode.outputs["Color"], mixSoftLightNode.inputs["Color2"])

    ### GREEN CHANNEL ###
    # Creates the color ramp node
    colorRampNodeG = material.node_tree.nodes.new(type = "ShaderNodeValToRGB")
    colorRampNodeG.location = (-850,200)
    material.node_tree.links.new(colorRampNodeG.outputs["Color"], mixShaderNode2.inputs["Color1"])
    ...
    material.node_tree.links.new(mixSoftLightNodeG.outputs["Color"], colorRampNodeG.inputs["Fac"])
    ...
    material.node_tree.links.new(ditherGroupNodeG.outputs["Color"], mixSoftLightNodeG.inputs["Color2"])

    ### BLUE CHANNEL ###
    # Creates the color ramp node
    colorRampNodeB = material.node_tree.nodes.new(type = "ShaderNodeValToRGB")
    colorRampNodeB.location = (-850,-100)
    material.node_tree.links.new(colorRampNodeB.outputs["Color"], mixShaderNode2.inputs["Color2"])
    ...
    material.node_tree.links.new(mixSoftLightNodeB.outputs["Color"], colorRampNodeB.inputs["Fac"])
    ...
    material.node_tree.links.new(ditherGroupNodeB.outputs["Color"], mixSoftLightNodeB.inputs["Color2"])
    ...
    material.node_tree.links.new(shaderToRgbNode.outputs["Color"], separateColorNode.inputs["Color"])
    material.node_tree.links.new(separateColorNode.outputs["Red"], mixSoftLightNode.inputs["Color1"])
    material.node_tree.links.new(separateColorNode.outputs["Green"], mixSoftLightNodeG.inputs["Color1"])
    material.node_tree.links.new(separateColorNode.outputs["Blue"], mixSoftLightNodeB.inputs["Color1"])
